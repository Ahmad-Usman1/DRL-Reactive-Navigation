...
Finetune_V3.py
==============
Finetuning for the V3 dual-delay architecture.

REWARD STRUCTURE
────────────────
Ported from the working FineTunePPO.py (V1) which achieved 75%+ success.
The only V3-specific addition is effective_front: diagonal rays (15°–30°)
are folded into the forward clearance estimate so corner walls register
3–5 steps earlier than the 0° ray sees them.

V1 elements preserved exactly:
  • Weighted-blend budget: front*0.55 + side*0.45  (NOT min())
  • Velocity excess coefficient: 20  (NOT 100)
  • SPEED_SCALE_DIST: 3.0, SIDE_TIGHTNESS_SCALE: 2.0
  • Heading penalty gated by budget_ratio + 15-step grace period
  • Narrow passage bonus at min_dist < 0.8m
  • Identical terminal rewards

WHY min() + coef=100 FAILED (the "stall at spawn" bug in the image)
  With SIDE_TIGHTNESS_SCALE=0.5, side_ratio saturates at 1.0 for any
  side_dist > 0.5m. But sensor noise can momentarily spike side_dist below
  0.5m, dropping budget_ratio to near 0 via min(). At coef=100 the penalty
  at that step is 100 × v² >> progress reward, so the gradient points toward
  v=0. Once stopped, the existence tax (−0.05/step) and zero progress mean
  there is no gradient to restart. The bot is trapped at spawn.
  Weighted blend with coef=20 produces a smooth gradient at all distances;
  the stop-oscillation trap cannot form.

HYPERPARAMETERS vs PREVIOUS RUN
  lr:       4e-5 → 8e-5  (policy was frozen by ~800k steps at 4e-5)
  n_epochs: 5    → 8     (more signal per rollout; safe with clip=0.1)
  ent_coef: 0.015 → 0.01 (less exploration needed at 75% base success)
  FT_TIMESTEPS: 4M → 6M  (previous policy stopped learning before 4M)
"""
...
PRETRAINED_MODEL_PATH = "models_beans_v3\\BEANS_V3_4100000_steps"
FT_LOG_DIR   = "./finetune_tensorboard_v3/"
FT_MODEL_DIR = "./finetune_models_v3/"
...
class FineTuneBotEnv_V3(PeopleBotEnv):
    ...
    def __init__(self):
        ...
        self.ft_map_bank = FineTuneMapBank(
            ...
            standard_difficulty=0.75   # match pretrained difficulty
        )
        self.waypoint_radius = 1.5   # match base env — 1.2 caused corner-cutting
        self.goal_radius     = 0.9

        self.steps_since_checkpoint = 0
        self.ft_previous_distance   = 0.0

        # Diagonal rays: 15°–30° band, both sides
        # Sensor order: [90,50,30,25,20,15,10,5,0,-5,-10,-15,-20,-25,-30,-50,-90]
    ...
    def step(self, action):
        ...
        # V3 obs: [scan(17), dist, head, v, w, del_v, del_w, act(2)]
        ...
        # Grace period is tracked by the parent step() via steps_since_checkpoint.
        # We do NOT duplicate it here — super().step() already updated it.

        reward = self._compute_ft_reward(
            scan_data=scan_data, heading_error=heading_error,
            dist_improvement=dist_improvement, hit_checkpoint=hit_checkpoint,
            hit_final_goal=hit_final_goal, is_crashed=is_crashed, truncated=truncated,
        )
    ...
    def _compute_ft_reward(self, scan_data, heading_error, dist_improvement,
        ...
        """
        V1 weighted-blend reward + V3 diagonal corner fix.

        The weighted blend (front*0.55 + side*0.45) is the load-bearing
        element. It produces a smooth, graded speed signal at all distances.
        The old min() version produced a cliff: any sensor noise spike below
        SIDE_TIGHTNESS_SCALE=0.5m collapsed budget_ratio to 0, fired a
        coef=100 penalty, and trapped the policy at v=0 permanently.

        effective_front folds diagonal rays into the forward estimate so the
        velocity budget starts trimming 3–5 steps before a corner wall reaches
        the 0° ray, giving the motor model time to decelerate within the
        pipeline's combined 200ms camera + 100ms actuator lag.
        """
        ...
        # Diagonal weight 0.85: corner wall at 2m diagonally → effective_front=1.7m.
        # In a straight corridor diag≈front, so effective_front≈front*0.85,
        # which the raised SPEED_SCALE_DIST=3.0 compensates for.
        ...
        #
        # In open space (effective_front≈2.5m, side=3.0m):
        #   front_ratio = 2.5/3.0 = 0.83,  side_ratio = 1.0 (capped)
        #   budget = 0.83*0.55 + 1.0*0.45 = 0.91 → cap 0.36 m/s  ✓ fast
        #
        # In 2m corridor (effective_front≈2.5m, side_dist≈0.69m):
        #   side_ratio = 0.69/2.0 = 0.35
        #   budget = 0.83*0.55 + 0.35*0.45 = 0.61 → cap 0.24 m/s  ✓ cautious
        #
        # At corner (effective_front=1.7m, side=0.69m):
        #   front_ratio = 1.7/3.0 = 0.57
        #   budget = 0.57*0.55 + 0.35*0.45 = 0.47 → cap 0.19 m/s  ✓ slow
        #
        # Excess coef=20: at excess=0.1m/s → penalty=0.20 vs progress=0.40.
        # Bot must genuinely overspeed before penalty > progress. NOT zero velocity.
        ...
        # 4. TTC backstop — uses effective_front for corner awareness
        ...
        # steps_since_checkpoint is updated by the parent step() before we
        # call _compute_ft_reward, so it is always current.
        ...
        # 7. Terminal rewards (unchanged from V1)
        if hit_checkpoint:
...
def main():
    print("=" * 60)
    print(" BEANS V3 Finetuning (Revised)")
    print(f" Model : {PRETRAINED_MODEL_PATH}")
    print(f" Steps : {FT_TIMESTEPS:,}  |  Envs: {N_ENVS}")
    print("=" * 60)
    ...
    model = PPO(
        ...
        learning_rate   = linear_schedule(8e-5),  # was 4e-5 — policy froze at 800k
        n_epochs        = 8,                       # was 5 — more signal per rollout
        ent_coef        = 0.01,                    # was 0.015 — less exploration needed
        clip_range      = 0.1,                     # unchanged — prevents forgetting

        n_steps         = 4096,   # must match V3 base
        batch_size      = 64,
        gamma           = 0.995,
        gae_lambda      = 0.95,
        device          = "auto",
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        name_prefix = "BEANS_V3_FineTuned",
    )
    ...
    try:
        model.learn(
            ...
            tb_log_name         = "PPO_FineTune_V3_Run",
            reset_num_timesteps = True,
            progress_bar        = True,
        )
        save_path = os.path.join(FT_MODEL_DIR, "BEANS_V3_FineTuned_Final")
        model.save(save_path)
        print(f"\nFinal model saved → {save_path}")
    except KeyboardInterrupt:
        save_path = os.path.join(FT_MODEL_DIR, "BEANS_V3_FineTuned_Interrupted")
        model.save(save_path)
